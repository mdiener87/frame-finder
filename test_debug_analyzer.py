# test_debug_analyzer.py
import os
import tempfile
import numpy as np
from PIL import Image
import cv2
from debug_analyzer import process_videos
...
def test_debug_pipeline():
    """Test the debug processing pipeline"""
    ...
    print("Running debug processing pipeline...")
    results = process_videos(
        reference_paths=ref_paths,
        video_paths=video_paths,
        frame_interval=0.5,  # Process every 0.5 seconds
        lpips_threshold=0.8,  # Very lenient threshold for testing
        clip_threshold=0.1   # Very lenient threshold for testing
    )
...
if __name__ == "__main__":
    try:
        results = test_debug_pipeline()
        print("\n✓ Debug pipeline test completed!")
    except Exception as e:
        print(f"\n✗ Debug pipeline test failed: {e}")
        import traceback
        traceback.print_exc()