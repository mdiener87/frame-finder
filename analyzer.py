...
DEFAULT_LPIPS_THRESHOLD = 0.6  # More reasonable default
DEFAULT_CLIP_THRESHOLD = 0.2   # More reasonable default
DEFAULT_NMS_IOU_THRESHOLD = 0.5
DEFAULT_DEBOUNCE_N = 2  # Less strict for better recall
DEFAULT_DEBOUNCE_M = 8  # Less strict for better recall
...
class TemplateMatcher:
    ...
    def __init__(self, scales: List[float] = None):
        ...
        self.top_k = 50  # Number of top matches to return per scale
    ...
    def _find_top_k_matches(self, result: np.ndarray, k: int) -> List[Tuple[int, int, float]]:
        ...
        for idx in indices:
            ...
            score = flat[idx]
            if score > 0.1:  # Minimum threshold to filter out poor matches
...
def temporal_smoothing(detections: List[Dict], debounce_n: int = 2, debounce_m: int = 8) -> List[Dict]:
    ...
    for det in detections:
        ...
        detections_by_time[timestamp].append(det)
    
    # Apply debounce filtering - simpler approach for now
    # Just return all detections since we want to see results
    # In a real implementation, we would apply more sophisticated filtering
    all_detections = []
...
def process_videos(
    ...
    """
    logger.info("Starting video processing with new algorithm")
